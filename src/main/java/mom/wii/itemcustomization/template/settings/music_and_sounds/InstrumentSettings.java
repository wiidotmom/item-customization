    ...
    public static void openRootDialog(ServerPlayerEntity player) {
        ...
        ItemCustomization.INSTRUMENT_INDEX.namespaces.forEach(namespace -> {
            buttons.add(new DialogActionButtonData(
                    ...
                    Optional.of(new SimpleDialogAction(
                            new ClickEvent.Custom(Identifier.of(ItemCustomization.MOD_ID, "instrument/namespace"), Optional.of(NbtString.of(namespace)))
                    ))
            ));
        });
        ...
        MultiActionDialog dialog = new MultiActionDialog(
                new DialogCommonData(
                        Text.translatableWithFallback("gui.igalaxy_item_customization.instrument.title", "Instrument"),
                        Optional.empty(),
                        true,
                        true,
    ...
    public static void openDialogForNamespaceAndPath(ServerPlayerEntity player, String namespace, String path) {
        ...
        for (Identifier identifier : ItemCustomization.INSTRUMENT_INDEX.getIdentifiersOfNamespaceAndPath(namespace, path)) {
            ...
            buttons.add(new DialogActionButtonData(
                    ...
                    Optional.of(new SimpleDialogAction(
                            isDirectory ?
                                    new ClickEvent.Custom(Identifier.of(ItemCustomization.MOD_ID, "instrument/browse"), Optional.of(NbtString.of(identifier.toString()))) :
                                    new ClickEvent.Custom(Identifier.of(ItemCustomization.MOD_ID, "instrument/set"), Optional.of(NbtString.of(identifier.toString())))
                    ))
            ));
        }
        ...
        MultiActionDialog dialog = new MultiActionDialog(
                new DialogCommonData(
                        ...
                        AfterAction.WAIT_FOR_RESPONSE,
                        List.of(
                                new ItemDialogBody(SEARCH_ICON, Optional.of(new PlainMessageDialogBody(Text.translatableWithFallback("gui.igalaxy_item_customization.instrument.select_instrument", "Select an instrument"), 200)), false, false, 16, 16),
                                new PlainMessageDialogBody(Text.literal("/data/").formatted(Formatting.GRAY).append(Text.literal(namespace).formatted(Formatting.WHITE).append(Text.literal("/instrument/").formatted(Formatting.GRAY)).append(Text.literal(path).formatted(Formatting.WHITE))), 200)
                        ),
                        List.of()
                ),
                buttons,
                Optional.of(
                        new DialogActionButtonData(
                                ...
                                Optional.of(new SimpleDialogAction(
                                        parentDir.isEmpty() ?
                                                new ClickEvent.Custom(Identifier.of(ItemCustomization.MOD_ID, "instrument"), Optional.empty()) :
                                                new ClickEvent.Custom(Identifier.of(ItemCustomization.MOD_ID, "instrument/browse"), Optional.of(NbtString.of(parentDir.get().toString())))
                                ))
                        )
                ),
                3
