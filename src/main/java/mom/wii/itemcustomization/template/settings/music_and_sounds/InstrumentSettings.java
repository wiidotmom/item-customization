...
public class InstrumentSettings {
    ...
    public static void openDialogForNamespace(ServerPlayerEntity player, String namespace) {
        ArrayList<DialogActionButtonData> buttons = new ArrayList<>();
        ItemCustomization.INSTRUMENT_INDEX.getIdentifiersOfNamespace(namespace).forEach(identifier -> {
            buttons.add(new DialogActionButtonData(
                    new DialogButtonData(
        ...
        MultiActionDialog dialog = new MultiActionDialog(
                new DialogCommonData(
                        ...
                        List.of(
                                ...
                                new PlainMessageDialogBody(Text.literal("/data/").formatted(Formatting.GRAY).append(Text.literal(namespace).formatted(Formatting.WHITE).append(Text.literal("/instrument/").formatted(Formatting.GRAY))), 200)
                        ),
                        List.of()
                ),
                buttons,
